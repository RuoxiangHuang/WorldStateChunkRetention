...
def sp_attn_forward_causal(
    ...
    current_end_int = current_start + seq_lens_int
    if is_dynamic_kv_cache(kv_cache):
        ...
        x_local = flash_attention(query, k_cache, v_cache)
        ctx_tokens = int(k_cache.shape[1])
        if "global_end_index" in kv_cache:
            kv_cache["global_end_index"].fill_(current_end_int)
        ...
        if return_kv_segment:
            if _ORACLE_STATE["on"]:
                # SP path: each rank holds the full token sequence with LOCAL heads,
                # so per-segment mass here is a clean over-tokens sum; _record_oracle_mass
                # all-reduces across ranks to average over all heads.
                layer_idx = int(kv_cache.get("layer_idx", 0))
                if layer_idx % _ORACLE_STATE["probe_every"] == 0:
                    _record_oracle_mass(
                        kv_cache, query, k_cache, self.head_dim, v_cache=v_cache)
            return x_full, build_dynamic_kv_segment(key, v)
        return x_full
