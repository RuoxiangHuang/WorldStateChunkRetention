...
@torch.no_grad()
def compute_chunk_motion_score(
    current_chunk: torch.Tensor,
    previous_chunk: torch.Tensor | None,
    aggregation: str = "peak_mean",
    peak_ratio: float = 0.34,
) -> float:
    """
    Compute a motion score from chunk-level control embeddings.

    Args:
        current_chunk:  Current chunk control tensor, typically [B, C, F, H, W].
            In ``act`` mode the channel dim already concatenates Plucker
            direction with WASD action, so the score is sensitive to both
            orientation and translation deltas.
        previous_chunk: Previous chunk control tensor with the same layout.
        aggregation: One of ``"mean"`` (legacy), ``"max"``, or ``"peak_mean"``.
            ``peak_mean`` collapses channels by mean and spatial dims by max
            to a per-frame magnitude, then averages the top ``peak_ratio``
            fraction of frames. This preserves intra-chunk motion peaks that
            ``mean`` would dilute when only a few frames within the chunk
            move significantly.
        peak_ratio: Fraction of top-frames kept by ``peak_mean`` (>=1 frame).

    Returns:
        A scalar motion score. The first chunk returns ``inf`` so that it is
        always preserved by motion-adaptive eviction.
    """
    if previous_chunk is None:
        return float("inf")
    diff = (current_chunk.float() - previous_chunk.float()).abs()
    if aggregation == "mean":
        return diff.mean().item()
    if aggregation == "max":
        return diff.max().item()
    if diff.dim() >= 3:
        # Layout [B, C, F, H, W] -> per-frame magnitude [B, F]
        per_frame = diff.mean(dim=1).flatten(2).amax(dim=-1)
        num_frames = per_frame.shape[-1]
        k = max(1, int(math.ceil(num_frames * peak_ratio)))
        return per_frame.topk(k, dim=-1).values.mean().item()
    return diff.mean().item()
