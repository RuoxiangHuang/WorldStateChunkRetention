#!/bin/bash
# MoCE tier-ratio sweep: sink:recent:archive(min_keep). MoSaiC only, 12-clip clips_ratio subset.
...
for cfg in "${CFGS[@]}"; do
  ...
  OUT="$SWEEP/$tag"
  if ls "$OUT"/stats/*_MoSaiC.json >/dev/null 2>&1; then
  ...
  MA_KV_SINK_SIZE=$sink MA_KV_RECENT_WINDOW=$recent MA_KV_MIN_KEEP=$arch \
    N=4 U=2 GPU_BASE=0 METHODS=MoSaiC \
